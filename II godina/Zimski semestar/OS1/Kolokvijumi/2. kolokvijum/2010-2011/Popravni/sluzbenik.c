...
void signal_handler(int sig) {
    printf("Ukupno spremljenih paketa: %d\n", *num);
    exit(0);
}
...
int main() {
    ...
    struct sembuf opr_inc_mutex, opr_dec_mutex, opr_inc_product;
    ...
    opr_inc_product.sem_num = 1;
    opr_inc_product.sem_op = 1;
    opr_inc_product.sem_flg = 0;
    ...
    while (1) {
        ...
        sleep(5);
        (*num)++;
        printf("Spremio %d posiljku!\n", *num);

        status = semop(semid, &opr_inc_mutex, 1);
        validateReturnValue(status);

        status = semop(semid, &opr_inc_product, 1);
        validateReturnValue(status);
    }
